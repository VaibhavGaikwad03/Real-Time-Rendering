...
int main(void)
{
    ...
    for (i_vtg = 0; i_vtg < NUM_ROWS; i_vtg++)
    {
        ...
        {
            *(iArray_vtg[i_vtg] + j_vtg) = (i_vtg + 1) * (j_vtg + 1); // 'iArray[i_vtg]' Can Be Treated As 1D Array Using Pointers ...
        }
    }
    ...
    for (i_vtg = 0; i_vtg < NUM_ROWS; i_vtg++)
    {
        for (j_vtg = 0; j_vtg < NUM_COLUMNS; j_vtg++)
        {
            printf("*(iArray_vtg[%d] + %d) = %d \t \t at address (iArray_vtg[i_vtg] + j_vtg): %p\n", i_vtg, j_vtg, *(iArray_vtg[i_vtg] + j_vtg), (iArray_vtg[i_vtg] + j_vtg));
        }
        printf("\n\n");
    }
