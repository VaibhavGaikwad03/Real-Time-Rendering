...
{
    // function prototypes
    int va_FindNumber_vtg(int, int, va_list);
    ...
    count_vtg = va_FindNumber_vtg(num_to_be_found_vtg, num_vtg, numbers_list_vtg);
...
int va_FindNumber_vtg(int num_to_be_found_vtg, int num_vtg, va_list list_vtg)
{
    // variable declarations
    int count_of_num_vtg = 0;
    int n_vtg;

    // code
    while (num_vtg)
    {
        n_vtg = va_arg(list_vtg, int);
        if (n_vtg == num_to_be_found_vtg)
            count_of_num_vtg++;
        num_vtg--;
    }

    return (count_of_num_vtg);
}