...
void display(void)
{
    ...
    glTranslatef(0.0f, 0.0f, -12.0f);   // Move pyramid backward
    glRotatef(45.0f, 0.0f, 1.0f, 0.0f); // Rotate pyramid by 45 degrees around Y-axis

    glBegin(GL_TRIANGLES);
    // Draw your pyramid (same code you wrote for pyramid)
    ...
    // Calculate the cube's position on a circle
    float radius = 12.0f;
    float cubeX = radius * cosf(angleCubeCamera);
    float cubeZ = radius * sinf(angleCubeCamera) - 12.0f; // Move it along -12 on Z to match pyramid's position

    // Set the "camera" at cube's position and looking at (0,0,-12)
    gluLookAt(cubeX, 0.0f, cubeZ, // Eye kiva camera position
              0.0f, 0.0f, -12.0f, // Center of pyramid
              0.0f, 1.0f, 0.0f);  // Up vector (Y-axis)

    // Small translation to position the cube a bit ahead after gluLookAt
...
{
    // code
    angleCubeCamera += 0.005f;
    if (angleCubeCamera >= 2.0f * PI) // complete circle
        angleCubeCamera = 0.0f;
}
