...
// toggle shapes
BOOL bToggleTriangle = FALSE;
BOOL bToggleSquare = FALSE;
BOOL bToggleCircle = FALSE;
BOOL bToggleGraph = FALSE;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CHAR:
        ...
        case 't':
            if (bToggleTriangle)
            {
                bToggleTriangle = FALSE;
            }
            else
            {
                bToggleTriangle = TRUE;
            }
            break;
...
void DrawHorizontalLines(float ox, float oy)
{
    ...
    {
        glVertex2f(-1.0f + ox, 0.05f + oy + space);
        glVertex2f(1.0f + ox, 0.05f + oy + space);
...
void DrawNonFilledTriangle(float ox, float oy)
{
...
void DrawNonFilledSquare(float ox, float oy)
{
    glColor3f(1.0f, 1.0f, 0.0f);
...
void DrawNonFilledCicle(float ox, float oy)
{
...
void display(void)
{
    ...
    if (bToggleTriangle)
    {
        DrawNonFilledTriangle(0.0f, 0.0f);
    }

    if (bToggleSquare)
    {
        DrawNonFilledSquare(0.0f, 0.0f);
    }

    if (bToggleCircle)
    {
        DrawNonFilledCicle(0.0f, 0.0f);
    }
