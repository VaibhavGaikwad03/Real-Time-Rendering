...
void display(void)
{
    ...
    // afte drawing sphere we need to go back to the original to draw earth. hence, restore the matrix
    glPopMatrix();

    // code for earth
    // again save this matrix to let us to combine again
    glPushMatrix();

    // let the earth allow to revolve around the sun
    glRotatef((GLfloat)year, 0.0f, 1.0f, 0.0f);

    // keep earth at some distance from the sun
    ...
    // code for moon
    // let the moon allow to revolve around the earth
    glRotatef((GLfloat)moon, 0.0f, 0.0f, 1.0f);

    // keep moon at some distance from the earth
    glTranslatef(0.5f, 0.0f, 0.0f);

    // adjust the poles of sphere of moon
    glRotatef(180.0f, 1.0f, 0.0f, 0.0f);

    // set moon's polygon property
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
